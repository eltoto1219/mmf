...
from transformers.modeling_bert import (
    ACT2FN,  # did we add more  act funcs or are they already present?
    # BertConfig, # were there changes made to lxmert
    # BertEmbeddings, #  were there changes made to LXMERT?
    # BertIntermediate, # were there changes made to LXMERT?
    # BertLayerNorm, check line 218
    # BertLMPredictionHead, #  were there changes made to LXMERT?
    # BertOutput,  # were there changes made to LXMERT?
    BertPredictionHeadTransform,
    # BertPreTrainedModel, # were there changes to LXMERT?
    BertSelfOutput,
)
...
PRETRAINED_MODEL_ARCHIVE_MAP = {
    ...
    "bert-base-multilingual-uncased":
        "https://s3.amazonaws.com/models.huggingface.co/bert/\
                bert-base-multilingual-uncased.tar.gz",
    "bert-base-multilingual-cased":
        "https://s3.amazonaws.com/models.huggingface.co/bert/\
                bert-base-multilingual-cased.tar.gz",
    "bert-base-chinese":
...
def load_tf_weights_in_bert(model, tf_checkpoint_path):
    ...
    try:
        ...
        print(
            "Loading a TensorFlow models in PyTorch, \
    ...
    for name, array in zip(names, arrays):
        name = name.split("/")
        # adam_v and adam_m are variables
        # used in AdamWeightDecayOptimizer to calculated m and v
        ...
        for m_name in name:
            if re.fullmatch(r"[A-Za-z]+_\d+", m_name):
                l_expr = re.split(r"_(\d+)", m_name)
            else:
                l_expr = [m_name]
            if l_expr[0] == "kernel" or l_expr[0] == "gamma":
                pointer = getattr(pointer, "weight")
            elif l_expr[0] == "output_bias" or l_expr[0] == "beta":
                pointer = getattr(pointer, "bias")
            elif l_expr[0] == "output_weights":
                pointer = getattr(pointer, "weight")
            else:
                pointer = getattr(pointer, l_expr[0])
            if len(l_expr) >= 2:
                num = int(l_expr[1])
                pointer = pointer[num]
...
class BertPreTrainedModel(nn.Module):
    ...
    def __init__(self, config, *inputs, **kwargs):
        ...
        if not isinstance(config, BertConfig):
            raise ValueError("Parameter config in {} (config)\
                should be an instance of class BertConfig.\
    ...
    def init_bert_weights(self, module):
        ...
        if isinstance(module, (nn.Linear, nn.Embedding)):
            ...
            # TF version which uses truncated_normal for initialization
    ...
    def from_pretrained(
        ...
        **kwargs
        ...
        """
        Instantiate a BertPreTrainedModel from a pre-trained model file
        or a pytorch state dict.
        Download and cache the pre-trained model file if needed.

        Params:
            pretrained_model_name_or_path: either:
                - a str with the name of a pre-trained model
                ...
                - a path or url to a pretrained model archive containing:
                    ...
                    . `pytorch_model.bin` a PyTorch dump of a
                    BertForPreTraining instance
                - a path or url to a pretrained model archive containing:
                    . `bert_config.json` a configuration file for the model
                    . `model.chkpt` a TensorFlow checkpoint
            from_tf: should we load the weights from a locally saved
            TensorFlow checkpoint
            cache_dir: an optional path to a folder in which the pre-trained
            models will be cached.
            state_dict: an optional state dictionnary
            (collections.OrderedDict object) to use instead of Google pre-trained models
        ...
        try:
            ...
            # dont know what to do about cached path
            ...
            if pretrained_model_name_or_path == "bert-base-uncased":
                ...
                # dont know what to do about the logger
                logger.error(
        ...
        if os.path.isdir(resolved_archive_file) or from_tf:
            ...
            # dont know what to do about temo file
...
# double check that we have same specifications in our config
...
"""BERTIMGEMBEDDINGCOLNFIG
    ...
    image_features, image_locations
"""
...
"""VILBERTBASEMODELCLASSCONFIG
class ViLBERT(BaseModel):
MAKE SURE TO INCLUDE THIS DECORATOR: @registry.register_model("vilbert")

NOTE: ive included much of this class bc it contains valuable information to how
mmf works
#########################################
def __init__(self, config):
        super().__init__(config)
...
#NOTE: ive also included a lot of the vilbert forward method bc it contains
good info
INPUTS + FORWARD:
